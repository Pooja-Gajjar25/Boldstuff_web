class FacetFiltersForm extends HTMLElement {
  ...
  static renderProductGridContainer(html) {
    ...
    //ishi scripts
    const productCounts = document.getElementsByClassName(
      "collection-product-count",
    );
    const newProductCounts = new DOMParser()
      .parseFromString(html, "text/html")
      .getElementsByClassName("collection-product-count");

    if (productCounts.length > 0 && newProductCounts.length > 0) {
      for (let i = 0; i < productCounts.length; i++) {
        productCounts[i].innerHTML = newProductCounts[0].innerHTML;
      }
    }
    ...
    if (document.querySelectorAll("#FacetFiltersFormSidebar").length) {
      if (
        document.querySelectorAll(
          "#FacetFiltersFormSidebar [type='checkbox']:checked",
        ).length
      ) {
        document.querySelector(
          "#FacetFiltersFormSidebar .clear-all",
  ...
  static renderFilters(html, event) {
    ...
    facetsToRender.forEach((element) => {
      ...
      document
        .querySelector("#main-collection-filters")
        .querySelector(
          `.js-filter[data-index="${element.dataset.index}"]`,
        ).innerHTML = element.innerHTML;
    });

    const sidebarForm = document.querySelector("#FacetFiltersFormSidebar");
    const newForm = parsedHTML.querySelector("#FacetFiltersForm");
    if (sidebarForm && newForm) {
      sidebarForm.innerHTML = newForm.innerHTML;
    }
