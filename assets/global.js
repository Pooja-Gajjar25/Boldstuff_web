...
class IshiModalDialog extends HTMLElement {
  ...
  show(opener) {
    ...
    document.querySelector(".newsletter-popup-overlay")
      ? document
          .querySelector(".newsletter-popup-overlay")
...
class VariantSelects extends HTMLElement {
  ...
  updateMedia() {
    ...
    document
      .getElementById("main-media-container")
      .prepend(newMedia.cloneNode(true));
  }
  ...
  doIshiTasks() {
    ...
    if (document.getElementsByClassName("ishi-progress-content").length) {
      ...
      var maxqty = parseInt(
        document
          .getElementsByClassName("ishi-progress-content")[0]
      ...
      if (stock > 0 && stock <= maxqty) {
        ...
        var progressSpan = document
          .getElementById("ishi-progress-bar")
...
function setupWishlistButtons() {
  if (
    getTheCookie("wishlist") != null &&
    getTheCookie("wishlist") != "__" &&
    getTheCookie("wishlist") != ""
  ) {
    var str = String(getTheCookie("wishlist")).split("__");
    for (var i = 0; i < str.length; i++) {
      if (str[i] != "") {
        document.querySelector(".added-wishbutton-" + str[i]) != null
          ? (document.querySelector(
              ".added-wishbutton-" + str[i],
            ).style.display = "block")
          : "";
        document.querySelector(".default-wishbutton-" + str[i]) != null
          ? document.querySelector(".default-wishbutton-" + str[i]).remove()
          : "";
        document.querySelector(".loadding-wishbutton-" + str[i]) != null
          ? document.querySelector(".loadding-wishbutton-" + str[i]).remove()
          : "";
      }
    }
  }
...
function setGridLayout() {
  ...
  if (document.querySelectorAll("[data-id='" + gridCookie + "']").length) {
    ...
    gridCookie = document
      .querySelector(".collectiongrid-layout.grid-layout-default")
      .getAttribute("data-id");
    setTheCookie("collectiongrid-layout", gridCookie, 14);
  }
  var classNames = document.querySelector(
    "[data-id='" + gridCookie + "']",
  ).classList;
  document.querySelector("[data-id=" + gridCookie + "]").classList =
    classNames + " active";
  document
    ...
    .forEach((element) => {
      element.className = document
        .querySelector("[data-id=" + gridCookie + "]")
        .getAttribute("data-class");
    });

  //list-view has the buttons and grid-view has been selected
  if (
    (gridCookie == "collectiongrid-layout-2" ||
      gridCookie == "collectiongrid-layout-3" ||
      gridCookie == "collectiongrid-layout-4") &&
    document
      .getElementById("product-grid")
      .querySelectorAll("li .card-information__wrapper .thumbnail-buttons")
      .length
  ) {
    document
      .getElementById("product-grid")
  ...
  if (
    gridCookie == "collectiongrid-layout-1" &&
    document
      .getElementById("product-grid")
      .querySelectorAll("li .card__inner .thumbnail-buttons").length
  ) {
    document
      .getElementById("product-grid")
      .querySelectorAll("li")
  ...
  document.querySelector("#shopify-section-Ishi_sidebar")
    ? (gridCookie += " with-sidebar")
    : "";
  document.getElementById("ProductGridContainer").className = gridCookie;
}
//cart upsell carousell function
function cartUpsellCarousel() {
  var rtlFlag = $("body").hasClass("lang-rtl") ? true : false;

  $("#cart-notification-slider").owlCarousel({
    nav: false,
    navText: [
...
function setCountDownTimer(date, target) {
  ...
  setInterval(function () {
    ...
    target.getElementsByClassName("countdown-days")[0].innerHTML =
      days < 10 ? "0" + days : days;
    target.getElementsByClassName("countdown-hours")[0].innerHTML =
      hours < 10 ? "0" + hours : hours;
    target.getElementsByClassName("countdown-minutes")[0].innerHTML =
      minutes < 10 ? "0" + minutes : minutes;
    target.getElementsByClassName("countdown-seconds")[0].innerHTML =
...
//Motivator Product JS Start
var motivatorProduct = false;
document.addEventListener("DOMContentLoaded", function () {
  document.addEventListener("click", function (event) {
    var target = event.target;
    if (target.classList.contains("click_gift")) {
      var isChecked = target.checked;
      fetch("/cart.json")
        .then((response) => response.json())
        .then((data) => {
          var items = data.items;
          var foundMotivatorProduct = items.some(
            (item) => item.handle === motivatorProductHandle,
          );
          if (isChecked && !foundMotivatorProduct) {
            addMotivatorProduct(motivatorProductVariantID);
          } else if (!isChecked) {
            removeMotivatorProduct(motivatorProductVariantID);
          }
          target.closest(".cart_loading").classList.add("cart_loading");
        });
    }
  });
  ...
  function cartItemCheck() {
    fetch("/cart.json")
      ...
      .then((data) => {
        ...
        if (items.length > 1) {
          removeMotivatorProduct(motivatorProductVariantID);
          document.querySelector(".click_gift").checked = false;
        }
      });
  }

  function initMotivatorProduct() {
    fetch("/cart.json")
      .then((response) => response.json())
      .then((data) => {
        var items = data.items;
...
function addMotivatorProduct(motivatorProductVariantID) {
  var xhr = new XMLHttpRequest();
  var url = "/cart/add.js";
  xhr.open("POST", url, true);
  xhr.setRequestHeader("Content-Type", "application/json");
...
function removeMotivatorProduct(motivatorProductVariantID) {
  var xhr = new XMLHttpRequest();
  var url = "/cart/change.js";
  xhr.open("POST", url, true);
  xhr.setRequestHeader("Content-Type", "application/json");
